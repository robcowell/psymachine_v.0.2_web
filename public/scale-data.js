/**
 * Scale and chord data ported from scale.lua (Renoise Scale Finder).
...
(function (global) {
  ...
  /** Returns boolean array [1..12] for which notes are in the scale. Root 1–12. */
  ...
  /** Returns array of scale note names in order (e.g. ['C','D','D#','F','G','G#','B']). */
  function getScaleNotes(root, scale) {
    const pattern = getScalePattern(root, scale);
    const out = [];
  ...
  /** Scale degree indices (1–12) in order: first 7 scale notes. */
  ...
  function isChordInScale(chordRootNote, chordIdx, scalePattern) {
    ...
    const cpat = c.pattern;
    for (let i = 0; i < cpat.length; i++) {
      if (cpat[i] !== '1') continue;
  ...
  /** Chord note names for chord built on chordRootNote (1–12). */
  ...
  function getChordLabel(chordRootNote, chordIdx) {
    ...
    const code = c.code;
    if (code === 'maj') return rootName;
    if (code === 'min') return rootName + 'min';
    return rootName + code;
  }

  /** For each scale degree (1–7), list of { label, chordRootNote, chordIdx } that are valid in scale. */
  function getValidChordsForScale(root, scale) {
    ...
    for (let d = 0; d < degreeIndices.length; d++) {
      const chordRootNote = degreeIndices[d];
      ...
      result.push({ roman: roman[d], chords: chordsInDegree });
    }
    return result;
  }
