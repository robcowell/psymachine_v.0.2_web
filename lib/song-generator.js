...
const { generateBass } = require('./bass-generator');
...
function generateTrackCellsForPattern(params, resolved, ctx, trackKey, patternIndex, melodyPool, bassPool) {
  ...
  function mask(cells, key) {
    ...
    return applyPatternDensityMask(cells, key, density, seededRandom(deriveSeed(patternSeed, `${key}-mask`)));
  }
  ...
  if ((trackKey === 'lead' || trackKey === 'lead2') && tracksConfig[trackKey]?.enabled !== false) {
    if (ctx.barBased && ctx.composition?.parentMotif) {
      ...
      const cells = generateLeadTrack(ctx, patternIndex, linesPerPattern, lpb, melodyPool, role);
      return { cells: mask(cells, trackKey) };
    }
  }

  if (trackKey === 'lead' && tracksConfig.lead?.enabled !== false) {
  ...
  if (trackKey === 'pad' && tracksConfig.pad?.enabled !== false) {
    if (ctx.barBased && ctx.chordTimeline) {
      const padRng = seededRandom(deriveSeed(patternSeed, 'pad'));
      ...
      const cells = generatePad(
        ...
        lpb,
        padRng,
        { includeThird: section.energy >= 40 && section.energy < 80 }
      );
  ...
  if (trackKey === 'kick' && tracksConfig.kick?.enabled !== false) {
    const cells = generateKick(linesPerPattern, lpb, seededRandom(deriveSeed(patternSeed, 'kick')), {});
    return { cells: mask(cells, 'kick') };
  }

  if (trackKey === 'snare' && tracksConfig.snare?.enabled !== false) {
    const snareOpts = ctx.barBased
      ? { ...genOpts, useClap: params.useClap !== false }
      : { useClap: params.useClap !== false, rollFillP: parseInt(params.snareRollP, 10) || 30 };
  ...
  if (trackKey === 'hihat' && tracksConfig.hihat?.enabled !== false) {
    const hatOpts = ctx.barBased
      ? genOpts
      : { openHatP: parseInt(params.openHatP, 10) || 25, skipP: parseInt(params.hatSkipP, 10) || 12 };
    const cells = generateHiHats(linesPerPattern, lpb, seededRandom(deriveSeed(patternSeed, 'hihat')), hatOpts);
  ...
  if (trackKey === 'perc' && tracksConfig.perc?.enabled) {
    const cells = generatePercussion(linesPerPattern, lpb, seededRandom(deriveSeed(patternSeed, 'perc')), genOpts);
...
function generateSongProject(params) {
  ...
  for (let p = 0; p < patternCount; p++) {
    ...
    const patternTracks = trackMeta.map(t => {
      const result = generateTrackCellsForPattern(params, resolved, ctx, t.key, p, melodyPool, bassPool);
  ...
  const noteCounts = trackMeta.map((t, i) => {
    ...
    patterns.forEach(pat => {
      total += pat[i].cells.filter(c => c && c !== 'OFF').length;
    });
    return total;
  });
