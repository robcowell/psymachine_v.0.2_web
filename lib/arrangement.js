...
/**
 * Apply arrangement density mask to a track (bar-block gating per section).
 */
function applyArrangementMask(cells, trackLength, arrangementId, trackKey, rng, options = {}) {
  const { applyBarBlockGate } = require('./arrangement-pattern');
  ...
  for (const section of arr.sections) {
    ...
    if (density <= 0) {
      ...
      const sectionBars = Math.max(1, Math.ceil((endLine - startLine) / barLines));
      const slice = result.slice(startLine, endLine);
      const gated = applyBarBlockGate(slice, density, barLines, rng, 0, sectionBars);
      for (let i = 0; i < gated.length; i++) result[startLine + i] = gated[i];
    }
  }
  return result;
}
