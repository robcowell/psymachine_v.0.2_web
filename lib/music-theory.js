...
/** Base octave for bass, drums, pads, and other non-lead elements. */
const NON_MELODIC_OCTAVE = 4;

/**
 * Bass note pool: root, fifth, and passing tones at NON_MELODIC_OCTAVE.
 */
function getBassNotePool(keyName, scaleIdx) {
  const root = keyToRoot[keyName];
  const scale = findScaleByIndex(parseInt(scaleIdx, 10) || 0);
  const oct = NON_MELODIC_OCTAVE;
  if (root == null || !scale) return { rootNote: 'E-4', fifthNote: 'B-4', passingNotes: [] };
