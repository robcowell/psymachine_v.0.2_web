...
function chordToBassPool(chord) {
  ...
  const tones = (chord.noteTokens?.length ? chord.noteTokens : [chord.rootNote, chord.thirdNote, chord.fifthNote])
    .filter(Boolean)
    .map(withOct);
  const unique = [...new Set(tones)];
  const rootNote = withOct(chord.rootNote);
  const fifthNote = withOct(chord.fifthNote || chord.rootNote);
  const thirdNote = chord.thirdNote ? withOct(chord.thirdNote) : rootNote;
  return { rootNote, fifthNote, thirdNote, passingNotes: unique.length ? unique : [rootNote, thirdNote, fifthNote] };
}
