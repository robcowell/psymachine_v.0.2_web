...
function energyBucket(energy) {
  if (energy <= 25) return 'lo';
  if (energy <= 50) return 'mid';
  if (energy <= 75) return 'hi';
  return 'peak';
}

function cacheKey(seedVal, sectionName, energy) {
  return `${seedVal}:${sectionName}:${energyBucket(energy)}`;
}
...
function resolveArchetypeBundle(ctx, resolved, patternIndex) {
  ...
  const key = cacheKey(sec.seedVal, sec.sectionName, sec.energy);
...
function getLeadRhythmArchetypeId(rhythmPlan, phraseSlot) {
  ...
  if (phraseSlot === 'B' && energy >= 50) {
    ...
    if (ids.includes('L7')) return 'L7';
    return ids[0] || 'L5';
  }
  if ((phraseSlot === 'A' || phraseSlot === "A'") && energy >= 50) {
    if (energy >= 75 && ids.includes('L2')) return 'L2';
    if (energy >= 60 && ids.includes('L7')) return 'L7';
    if (ids.includes('L1')) return 'L1';
    if (energy < 40 && ids.includes('L4')) return 'L4';
    return ids[0] || 'L1';
  }
  if (phraseSlot === 'C' && ids.includes('L3')) return 'L3';
  if (!ids.length) return energy < 25 ? 'L4' : 'L1';
  return ids[0];
}

const ROLLING_INCOMPATIBLE_BASS = new Set(['B1', 'B4', 'B8']);

function getBassArchetypeId(rhythmPlan) {
  const ids = rhythmPlan.archetypes.bass || [];
  let id = ids.length ? ids[0] : (LEGACY_BASS_STYLE_MAP[rhythmPlan.bassStyle] || 'B1');
  if (rhythmPlan.bassStyle === 'rolling' && ROLLING_INCOMPATIBLE_BASS.has(id)) {
    return LEGACY_BASS_STYLE_MAP.rolling;
  }
  return id;
}
