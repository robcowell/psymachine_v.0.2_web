...
const { buildSingleTrackXml } = require('./renoise-xml');
const { randomPercent, seededRandom } = require('./rng');
 ...
 * @returns {{ cells: string[], error?: string }}
 */
function generateMelodyCells(params) {
  const {
    baseNote,
    otherNotes,
...
function generate(params) {
  const result = generateMelodyCells(params);
  if (result.error) return result;
  const trackLength = parseInt(params.trackLen, 10) || 64;
  const xml = buildSingleTrackXml(trackLength, result.cells, params.instrumentNumber ?? '0');
  return { xml };
}

module.exports = { generate, generateMelodyCells, parseNotes, superTrim, normalizeRenoiseNoteToken };
