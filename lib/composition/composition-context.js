...
function normalizeSelectedChords(params) {
  if (!params?.selectedChords) return [];
  const raw = params.selectedChords;
...
function createCompositionContext(params) {
  ...
  if (!bassStyle) {
    if (useStyleDefaults && style?.bassStyle) {
      bassStyle = style.bassStyle;
    } else {
      bassStyle = style?.bassStyle || 'rolling';
    }
  }
  ...
  const selectedChords = normalizeSelectedChords(params);
  const chordTimeline = barBased
    ? buildChordTimeline(composition?.timeline || barTimeline, keyName, scaleIdx, 4, { selectedChords })
    : null;
