...
const { barLengthLines, applyPhraseRole } = require('./motif-generator');
 ...
 * @param {'question'|'answer'} [options.phraseRole] — call-and-response half (Ch. 22)
 */
function renderPhraseStructure(phraseSlots, motifFamily, parentCells, trackLength, lpb, options = {}) {
  ...
  if (options.phraseRole) {
    return applyPhraseRole(out, lpb, options.phraseRole);
  }
  return out;
}
