...
function generateParentMotif(melodyPool, seedVal, lpb, rng) {
  ...
  const noteCount = 8 + ((rng() * 3) | 0);
  const contour = pickContour(rng, rng() < 0.35 ? 'orbit' : null);
  ...
  const barLines = barLengthLines(lpb);
  const rhythmOffsets = hookRhythm(pitchDegrees.length, barLines, lpb, rng, 'L2');
...
function deriveMotifFamily(parent, seedVal, lpb) {
  ...
  const responseDegrees = shiftDegrees(
    reverseDegrees(pitchDegrees),
    rng() < 0.5 ? 2 : 4,
    max
  );
  const response = renderMotifEvents(responseDegrees, rhythmOffsets, 1, lpb, scaleNotes, { rng });
